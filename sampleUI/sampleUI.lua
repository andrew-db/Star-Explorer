...
function M:newUI( options )
	...
	local textGroupContainer = display.newContainer( 552, 480 ) ; frontGroup:insert( textGroupContainer )
	local barContainer = display.newContainer( display.actualContentWidth, 60 )
	...
	-- Place Corona title
	local siteLink = display.newText( barContainer, "Corona Labs", 16, topBarOver.contentHeight / 2, useFont, 24 )
	siteLink.anchorX = 0
	...
	local title = display.newText( barContainer, sampleCodeTitle, barContainer.contentWidth - 56, topBarOver.contentHeight / 2, useFont, 28 )
	title.anchorX = 1

	if options.showBuildNum == true then
		buildNum = display.newText( frontGroup, "Build " .. tonumber( system.getInfo( "build" ):sub(-8) ), 0, 0, useFont, 20 )
		buildNum.anchorX = 1
		buildNum.anchorY = 1
	...
	local infoButton = display.newImageRect( barContainer, "sampleUI/infobutton.png", 50, 50 )
	...
	local function onResize( event )
		...
		title.x = barContainer.contentWidth - 56
		infoButton.x = barContainer.contentWidth - 3
		...
		objPos["infoBoxOffY"] = display.screenOriginY - 260
		objPos["infoBoxDestY"] = (barContainer.y + barContainer.contentHeight + 260)
	...
	if readMeText ~= "" then
		...
		local infoBoxBack = display.newRect( 0, 0, 576, 480 )
		...
		local infoText = display.newText( infoTextGroup, "", 0, 0, 520, 0, useFont, 24 )
		...
		if #docLinks > 0 then
			for i = 1,#docLinks do
				local docLink = display.newText( docLinks[i][1], 0, 0, useFont, 24 )
		...
		local spacer = display.newRect( infoTextGroup, 0, infoTextGroup.contentBounds.yMax, 20, 30 )
		spacer.anchorY = 0 ; spacer.isVisible = false

		local infoTextAnchorBox = display.newRect( infoTextGroup, 0, 0, 576, math.max( 480, infoTextGroup.height ) )
		...
		transComplete = function()
			...
			if infoBoxState == "opening" then
				...
				infoTextGroup.x = 288 ; infoTextGroup.y = 240
